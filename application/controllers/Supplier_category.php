...
class Supplier_category extends CI_Controller
{
    ...
    public function index()
    {
        if($this->session->userdata('id_position')!=NULL && $this->session->userdata('id_position') == 1){        
            $supplier_category = $this->Supplier_category_model->get_all_detail();
    ...
    {
        $row = $this->Supplier_category_model->get_by_id_detail($id);
        if ($row) {
            $data = array(
	...
	foreach ($this->Supplier_category_model->get_all_detail() as $data) {
            $kolombody = 0;
    ...
    public function word()
    {
        ...
        $data = array(
            'supplier_category_data' => $this->Supplier_category_model->get_all_detail(),
            'start' => 0
        );
