...
class Form extends CI_Controller
{
    ...
    public function acc(){
        ...
        foreach ($item_list as $item) {
            if(isset($_POST["$item->id_items_detail"])){
                $data = array(
                    'status_acc' => 1,
                    );
                $this->Form_content_model->update($item->id_items_detail,$data);
            }else{
                 $data = array(
                    'status_acc' => 0,
                    );
    ...
    public function autocompleteCat($item = NULL){
        $this->load->model('Items_category_model');
        ...
        $result = $this->Items_category_model->get_like($term,$item);
